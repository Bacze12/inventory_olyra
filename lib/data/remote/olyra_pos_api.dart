...
class OlyraSyncResult {
  ...
  final int shiftsPushed;
  final List<String> errors;

  bool get hasChanges =>
      salesPushed + movementsPushed + productsPushed + shiftsPushed > 0;

  String describe() {
    final parts = <String>[
      if (salesPushed > 0) '$salesPushed venta(s) subidas a la nube',
      if (movementsPushed > 0) '$movementsPushed movimiento(s) subidos',
      if (productsPushed > 0) '$productsPushed producto(s) del catálogo',
      if (shiftsPushed > 0) '$shiftsPushed turno(s) subidos',
      if (!hasChanges) 'Sin cambios pendientes.',
    ];
    if (errors.isNotEmpty) {
      parts.add('Con ${errors.length} advertencia(s) · 1ª: ${errors.first}');
    }
    return parts.join(' · ');
  }
}
...
class OlyraPosApi {
  ...
  /// Empuja en un solo lote las ventas, movimientos, catálogo y turnos locales.
  ///
  ...
  Future<OlyraSyncResult> syncNow({
    ...
    debugPrint('[OlyraSync] POST $_syncUrl '
        ...
        'products=${products.length} shifts=${shifts.length} '
        'user_app_id=$userAppId');
    ...
    final productsPushed =
        ((body?['synced_products'] ?? body?['synced_products_count']) as num?)
                ?.toInt() ??
            products.length;
    final shiftsPushed =
        (body?['synced_shifts'] as num?)?.toInt() ?? shifts.length;
    final errors = <String>[];

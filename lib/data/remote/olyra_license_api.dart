...
class LicenseServerException implements Exception {
  const LicenseServerException(this.message, {this.statusCode});
...
/// Cliente HTTP de activación de licencia (un solo uso online).
///
/// POST `{ license_key, hwid }` → `{ success: true, token: string }`.
/// El token emitido es un JWT RS256 verificado offline por la app.
class OlyraLicenseApi {
  OlyraLicenseApi({required String activationUrl, http.Client? client})
  ...
  Future<String> activate({
    ...
    final payload = jsonEncode({'license_key': normalized, 'hwid': hwid});
